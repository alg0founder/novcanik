...
function ChangeEmailModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async (e: FormEvent): Promise<void> => {
    e.preventDefault()
    setError(null)
    if (!newEmail.includes('@')) { setError('Unesite ispravnu email adresu.'); return }
    if (newEmail === user?.email) { setError('Nova adresa je ista kao trenutna.'); return }
    setLoading(true)
    const { error: authErr } = await supabase.auth.signInWithPassword({ email: user!.email!, password })
    if (authErr) { setError('Lozinka nije ispravna.'); setLoading(false); return }
    const { error: updateErr } = await supabase.auth.updateUser({ email: newEmail })
    if (updateErr) { setError('Greška pri promeni emaila.'); setLoading(false); return }
    setSuccess(true)
    setLoading(false)
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-sm bg-[#111418] border border-white/10 rounded-2xl shadow-2xl p-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-[#e1e2e7]">Promeni email</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 transition-colors">
            <X size={18} />
          </button>
        </div>
        {success ? (
          <div className="text-center py-4 space-y-3">
            ...
            <p className="text-sm text-[#e1e2e7] font-semibold">Potvrda je poslata na novu adresu</p>
            <p className="text-xs text-slate-500">Klikni link u emailu da završiš promenu.</p>
            <button onClick={onClose} className="w-full mt-2 py-2.5 rounded-full bg-orange-500 hover:bg-orange-600 text-[#2d1600] font-bold text-sm transition-all">
              Zatvori
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Nova email adresa</label>
              <input type="email" value={newEmail} onChange={e => setNewEmail(e.target.value)} placeholder="nova@adresa.com" className={inputCls} />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Potvrdi lozinkom</label>
